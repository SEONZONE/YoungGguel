...
<script type="text/javascript">
	$(function() {
		...
		/* 정렬 버튼 함수 */
		$(".align-button img").click(
				function() {
					var img = $(this).attr('src');
					if (img == '/movie/view/img/list.PNG') {
						$(".align-button img").attr("src",
								"/movie/view/img/poster.PNG");
						if (result == 'poster') {
							ajaxList('/movie/ajaxMovieList.do?view=poster',
									'json');
						} else if (result == 'current') {
							ajaxList('/movie/ajaxMovieList.do?view=current',
									'json');
						} else if (result == 'later') {
							ajaxList('/movie/ajaxMovieList.do?view=later',
									'json');
						}
					} else {
						/* 버튼이 리스트형일 때 */
						$(".align-button img").attr("src",
								"/movie/view/img/list.PNG");
						if (result == 'poster') {
							ajaxPoster('/movie/ajaxMovieList.do?view=poster',
									'json');
						} else if (result == 'current') {
							ajaxPoster('/movie/ajaxMovieList.do?view=current',
									'json');
						} else if (result == 'later') {
							ajaxPoster('/movie/ajaxMovieList.do?view=later',
									'json');
						}
					}
				});
	...
	function heartInsert(id, movieCd) {
		$.ajax({
			...
			success : function(heartSum) {
				...
				$('img[id="' + movieCd + '"]').attr("src",
						"/movie/view/img/heartClick.png");
		...
		$
				.each(
						...
						function(index, dom) {
							...
							temp += "<a href=\"/movie/movieInfo.do?no="
									+ dom.movieCd + "\">";
							temp += "<img src=\"/movie/view/img/"+dom.movieCd+".jpg\" id=\"mposter\" />";
							temp += "<br>";
							temp += "<span id=\"title\">" + dom.movieNm
									+ "</span>";
							temp += "<br>";
							temp += "<span id=\"release\">개봉일 " + dom.openDt
									+ " |</span>";
							temp += "<span id=\"runningtime\">" + dom.showTm
									+ "분</span>";
							temp += "<br>";
	...
	function ajaxMovieList(v) {
		...
		$.each(v, function(index, dom) {
			...
			temp += " <span>개봉 </span><span class=\"release\">" + dom.openDt
					+ "</span>";
		...
		<div class="inner-wrap">
			<div class="movie-list">
				<c:forEach var="i" items="${movieList}" varStatus="cnt">
					<div class="movie">
						<a href="/movie/movieInfo.do?no=${i.movieCd}"> <img src="/movie/view/img/${i.movieCd}.jpg" id="mposter" /> <br> <span id="title">${i.movieNm}</span> <br> <span
							id="release"
						>개봉일 ${i.openDt} |</span> <span id="runningtime">${i.showTm}분</span> <br> <span id="ratio">예매율 30%</span>
						</a> <br> <br>
						<button>예매하기</button>
						<div class="heart">
